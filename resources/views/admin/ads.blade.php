...
@section('content')
						...
						<ol class="inline-flex items-center space-x-1 md:space-x-2">
							...
							<li>
								<div class="flex items-center">
									...
									<span class="ml-1 text-sm font-medium text-gray-400 md:ml-2 line-clamp-1">Реклама</span>
								</div>
							</li>
						</ol>
					</nav>
					<h1 class="text-xl font-semibold text-gray-900 sm:text-2xl">Все рекламы</h1>
				</div>
				<div class="block items-center sm:flex md:divide-x md:divide-gray-100">
					<div class="flex items-center w-full sm:justify-end">
						<a href="javascript:addAd()" class="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white rounded-lg bg-blue-500 hover:bg-blue-600 sm:ml-auto hover:scale-[1.02] transition-all">
						...
						<table class="min-w-full divide-y divide-gray-200 table-fixed">
							...
							@foreach($ads as $ad)
								<tr class="hover:bg-gray-100">
									<td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap lg:p-5">#{{$ad->id}}</td>
									<td class="p-4 text-sm font-normal text-gray-500 lg:p-5">
										<div class="text-base font-semibold text-gray-900 w-[300px] line-clamp-1">{{$ad->title}}</div>
									</td>
									<td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap lg:p-5">{{$ad->created_at->isoFormat('LL')}}</td>
									<td class="p-4 space-x-2 whitespace-nowrap lg:p-5">
										<a href="javascript:deleteAd({{$ad->id}})" class="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-red-500 rounded-lg hover:bg-red-800 hover:scale-[1.02] transition-all">
	...
	<div id="addAdWindow" class="hidden overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-4 z-50 justify-center items-center md:inset-0 h-modal sm:h-full bg-gray-100 bg-opacity-75">
		<div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 w-full max-w-2xl h-full md:h-full">
			<div class="relative bg-white rounded-2xl shadow-lg">
				<div class="flex justify-between items-start p-5 rounded-t border-b">
					<h3 class="text-xl font-semibold">Добавить рекламу</h3>
					<a href="javascript:addAd()" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-full w-6 h-6 inline-flex items-center justify-center">
				...
				<form action="{{route('admin.ad.store')}}" method="POST" enctype="multipart/form-data">
					...
					<div class="p-6 rounded-b border-t border-gray-200">
						<button type="submit" class="text-white font-medium text-sm px-5 py-2.5 text-center rounded-lg bg-blue-500 hover:scale-[1.02] transition-all">Добавить рекламу</button>
					</div>
				</form>
			</div>
		</div>
	...
	<div id="deleteAdWindow" class="hidden overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-4 z-50 justify-center items-center md:inset-0 h-modal sm:h-full bg-gray-100 bg-opacity-75">
		<div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 w-full max-w-md h-full md:h-auto">
			<div class="relative bg-white rounded-2xl shadow-lg">
				<div class="p-6 pt-0 text-center">
					...
					<h3 class="mt-5 mb-6 text-xl font-normal text-gray-500">Вы уверены, что хотите удалить эту рекламу?</h3>
					<form id="deleteAdForm" action="" method="post" class="inline-flex">
						@csrf
						@method('delete')
					...
					<a href="javascript:deleteAd(0)" class="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 font-medium inline-flex items-center rounded-lg text-base px-3 py-2.5 text-center hover:scale-[1.02] transition-transform">
						Нет
					</a>
				</div>
			</div>
